...
void BFS(Graph G)
{
    ...
    // 开始遍历
    for (i = 0; i < G.vexnum; i++)  // 对所有顶点遍历
    {
        if (!visited[i])  // 如果没有访问过
        {
            visited[i] = 1;  // 访问标记
            printf("%c ", G.vexs[i]);  // 打印内容
            queue[rear++] = i;  // 入队列
        }
        while (head != rear)  // 如果列表为没有空
        {
            j = queue[head++];  // 出队列
            for (k = first_vertex(G, j); k >= 0; k = next_vertix(G, j, k)) //k是j顶点的所有未访问的邻接顶点
            {
                if (!visited[k])  // 如果k没有被访问
                {
                    visited[k] = 1;  // 访问标记
                    printf("%c ", G.vexs[k]);  // 打印
                    queue[rear++] = k;  // 入队
                }
            }
        }
